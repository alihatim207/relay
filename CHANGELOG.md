...
- `relay ls`, `relay usage` and the dashboard end with two freshness ages instead of one: `metrics 2s ago · job state 41s ago`. One "last synced" number had to be the older of the two, which made a healthy daemon look stalled. `--json` keeps `last_synced`/`last_synced_age_s` and adds `last_scheduler`/`last_scheduler_age_s`. With no active runs the line reads `no active runs · daemon alive Ns ago` instead, because an idle daemon makes no cluster calls at all and two fresh ages would look like polling for nothing. `--json` also carries `active_runs`.
