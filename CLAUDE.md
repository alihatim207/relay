...
Two schedules inside one loop, because they cost different things. The **tail** (event-log reads on the shared filesystem) is adaptive: 2 seconds when runs are actively emitting, 30 when everything is queued, 60 when nothing is active. The **scheduler poll** (`squeue`, one call for all runs) goes to slurmctld, which every user on the cluster shares, so it is held to at least `scheduler_interval_min` (default 30, hard floor 10, clamped with a warning) regardless of run state, and multiplies by 1.5 after every poll in which no run changed status, up to `scheduler_interval_max` (default 300). Any status change resets it to the minimum; `relay submit` and `relay cancel` reset it too by writing `scheduler_poke_ts` into `meta`, which the daemon compares against the last poke it acted on. A cycle does the tail if it is due and the poll if it is due; the sleep is the sooner of the two. `usage_interval` (default 300) is the `sacct` timer. All six live under `daemon:` in config and as `relay daemon` flags; a flag beats the file. A cycle that touched neither the filesystem nor the scheduler during a failure streak is neutral: it neither clears the streak nor stamps any freshness.

Two freshness stamps in `meta`: `last_sync_ts` (tail) and `last_scheduler_ts` (poll). `relay ls`, `relay usage` and the dashboard show both as `metrics Ns ago · job state Ns ago`; `--json` keeps `last_synced`/`last_synced_age_s` and adds `last_scheduler`/`last_scheduler_age_s`.
